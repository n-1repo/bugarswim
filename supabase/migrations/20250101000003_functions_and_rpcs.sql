...
create function owns_child(p_child_id uuid) returns boolean as $$
  select exists (
    ...
    where c.id = p_child_id and c.parent_id = auth.uid() and p.is_active
  );
$$ language sql stable security definer set search_path = public;
...
create function coach_owns_class(p_class_id uuid) returns boolean as $$
  select exists (
    ...
    where cl.id = p_class_id and cl.instructor_id = auth.uid() and p.is_active
  );
$$ language sql stable security definer set search_path = public;
...
begin
  ...
  where id = p_invoice_id and status = 'outstanding'
  ...
  insert into cash_ledger (category, direction, amount, invoice_id, created_by)
  values ('payment_received', 'in', v_amount, p_invoice_id, auth.uid());
end;
...
begin
  ...
  insert into payroll_runs (coach_id, period_start, period_end, base_salary, bonus, thr, status)
  values (p_coach_id, p_period_start, p_period_end, p_base_salary, p_bonus, p_thr, 'draft')
  returning id, total_amount into v_run_id, v_total;

  insert into cash_ledger (category, direction, amount, payroll_run_id, created_by)
  values ('payroll', 'out', v_total, v_run_id, auth.uid())
...
create function generate_invoices_for_period(
  ...
  select s.child_id, s.id, mp.price, p_due_date, p_period_start, p_period_end
...
grant execute on function generate_invoices_for_period(date, date, date) to service_role;
